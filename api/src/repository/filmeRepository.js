...
export async function alterarImagem(imagem, idFilme) {
    const comando = 
    `
    UPDATE tb_filme 
     SET img_filme     = ?
     WHERE id_filme = ?;
    `
    const [resposta] = await con.query(comando, [imagem, idFilme]);
...
export async function alterarFilme(id, filme) {
    ...
    const [resposta] = await con.query(comando, [filme.nome, filme.sinopse, filme.avaliacao, filme.lancamento, filme.usuario, filme.disponivel, id]);
    return resposta.affectedRows;
}